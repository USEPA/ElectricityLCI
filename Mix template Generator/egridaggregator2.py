# -*- coding: utf-8 -*-
"""
Created on Mon Aug  7 21:45:12 2017
...
chk1 = egrid[[' eGRID subregion acronym ', ' Plant annual net generation (MWh) ']].groupby([' eGRID subregion acronym ',]).sum()
#chk2 = egrid[[' eGRID subregion acronym ', ' Plant annual net generation (MWh) ']].groupby([' eGRID subregion acronym ',' Plant primary coal/oil/gas/ other fossil fuel category ']).sum()
chk1.reset_index( drop=False, inplace=True)
#chk2.reset_index( drop=False, inplace=True)

#Trying to Just compile EGRID  for emissions based on Regions fuel sources first

chk1.to_csv('Aggregated_LCI.csv')
...
chk3 = egrid[[' eGRID subregion acronym ', ' Plant annual net generation (MWh) ']].groupby([' eGRID subregion acronym ']).sum()
chk4 = egrid[[' eGRID subregion acronym ', ' Plant annual net generation (MWh) ']].groupby([' eGRID subregion acronym ']).sum()
