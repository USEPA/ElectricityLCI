...
def list_facilities_w_percent_generation_from_primary_fuel_category_greater_than_min(egrid_facilities_fuel_cat_per_gen, min_plant_percent_generation_from_primary_fuel_category=90):
...
def get_egrid_facilities(egrid_year):
    ...
    return egrid_facilities

def get_egrid_subregions():    
    subregions = pd.read_csv(join(data_dir, 'egrid_subregion_to_NERC.csv'), low_memory=False)
    egrid_subregions = list(pd.unique(subregions['Subregion']))
