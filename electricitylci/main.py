...
def main():
    ...
    if model_specs.include_upstream_processes is True:
        ...
        upstream_df = electricitylci.get_upstream_process_df(model_specs.eia_gen_year)
        ...
        generation_process_df = electricitylci.get_generation_process_df(model_specs,
    ...
    if model_specs.regional_aggregation in ["FERC","US"]:
        generation_mix_df = electricitylci.get_generation_mix_process_df(model_specs, "BA")
    else:
        generation_mix_df = electricitylci.get_generation_mix_process_df(model_specs)
    print("write gen mix to dict")
    generation_mix_dict = electricitylci.write_generation_mix_database_to_dict(model_specs,
        generation_mix_df, generation_process_dict)
    print("write gen mix to jsonld")
    generation_mix_dict = electricitylci.write_process_dicts_to_jsonld(model_specs,
        generation_mix_dict
    )
    ...
    if model_specs.EPA_eGRID_trading is False:
        ...
        cons_mix_df_dict = electricitylci.get_consumption_mix_df(model_specs)
        print("write consumption mix to dict")
        cons_mix_dicts={}
        for subreg in cons_mix_df_dict.keys():
            # NEED TO FIND A WAY TO SPECIFY REGION HERE
            cons_mix_dicts[subreg] = electricitylci.write_consumption_mix_to_dict(model_specs,
        ...
        for subreg in cons_mix_dicts.keys():
            cons_mix_dicts[subreg] = electricitylci.write_process_dicts_to_jsonld(model_specs,
                cons_mix_dicts[subreg])
        print("get distribution mix")
        dist_mix_df_dict={}
        for subreg in cons_mix_dicts.keys():
            dist_mix_df_dict[subreg] = electricitylci.get_distribution_mix_df(model_specs,
                generation_process_df,subregion=subreg)
        print("write dist mix to dict")
        dist_mix_dicts={}
        for subreg in dist_mix_df_dict.keys():
            dist_mix_dicts[subreg] = electricitylci.write_distribution_mix_to_dict(model_specs,
        ...
        for subreg in dist_mix_dicts.keys():
            dist_mix_dicts[subreg] = electricitylci.write_process_dicts_to_jsonld(model_specs,
        ...
        # Get dist dictionary
        dist_dict = electricitylci.write_distribution_dict(model_specs)
        generation_mix_dict = electricitylci.write_process_dicts_to_jsonld(model_specs,
            generation_mix_dict
        )
