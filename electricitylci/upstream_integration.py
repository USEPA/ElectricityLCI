...
column_filt = (eia_generation_data['Reported Fuel Type Code'] == 'NG') & (eia_generation_data['Elec Fuel Consumption MMBtu'] > 0)

ng_generation_data = eia_generation_data[column_filt]

ng_generation_data = ng_generation_data.groupby('Plant Id').agg({'Elec Fuel Consumption MMBtu':'sum'}).reset_index()
...
ng_generation_data_basin = pd.merge(left = ng_generation_data, right = ng_basin_mapping, left_on = 'Plant Id', right_on = 'Plant Code')

ng_generation_data_basin = ng_generation_data_basin.drop(columns = ['Plant Code'])

#Read the NG LCI excel file
ng_lci = pd.read_excel(data_dir + 'NG_LCI.xlsx', sheetname = 'Basin_Mean_Data')
...
# This line can go away once the elementary flow names are replaced in the NG_LCI.xlsx file.
...
ng_lci_basin = pd.merge(ng_lci, ng_generation_data_basin, left_on = 'Basin', right_on = 'NG_LCI_Name')


#Multiplying with the EIA 923 fuel consumption; conversion factor is for MMBtu to MJ
for i in emissions_list:
    ng_lci_basin[i] = ng_lci_basin[i] * ng_lci_basin['Elec Fuel Consumption MMBtu'] * 1055.056
    
ng_lci_basin = ng_lci_basin.drop(['NG_LCI_Name', 'Elec Fuel Consumption MMBtu'], axis = 1)

#Output is kg emission for 2016 by facility Id, not normalized to electricity output
ng_lci_basin_melt = ng_lci_basin.melt(id_vars = ['Plant Id', 'Compartment','Stage'],value_vars = emissions_list,var_name = 'FlowName',value_name = 'FlowAmount')

#Aggregate process stages 'PRODUCTION', 'GATHERING & BOOSTING', 'PROCESSING', 'TRANSMISSION',
#'STORAGE', 'PIPELINE' to just "extraction" and "transportation"

ng_stage_dict = {'PRODUCTION':'extraction', 'GATHERING & BOOSTING':'extraction','PROCESSING':'extraction', 'TRANSMISSION':'transportation', 'STORAGE':'transportation', 'PIPELINE':'transportation'}
ng_stage_df = pd.DataFrame.from_dict(ng_stage_dict, orient = 'index').reset_index()
ng_stage_df = ng_stage_df.rename(index=str, columns={"index": "process_detail", 0: "stage"})

ng_lci_basin_melt = pd.merge(ng_lci_basin_melt, ng_stage_df, left_on = 'Stage', right_on = 'process_detail')

#Group emissiosn by new stage designation - extraction and transportation
ng_lci_basin_grouped = lci_basin_melt.groupby(['Plant Id', 'FlowName','stage']).agg({'FlowAmount':'sum'}).reset_index()
