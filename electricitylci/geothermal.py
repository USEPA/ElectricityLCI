...
def generate_upstream_geo(year):
    ...
    geo_generation_data = geo_generation_data.groupby(['Plant Id','State']).agg(
            {'Net Generation (Megawatthours)':'sum'}).reset_index()
    geo_generation_data['Plant Id'] = geo_generation_data['Plant Id'].astype(int)
