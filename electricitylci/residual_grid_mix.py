...
def get_rem(rec_handler='zero', agg_handler='count', to_save=False):
    ...
    m_df = get_elci_mix()
    m_df = update_mix(m_df, rec_handler, agg_handler)
...
def update_mix(df, rec_handler='zero', agg_handler='count'):
    ...
    agg_df = get_rec_agg(agg_handler, as_series=False)
