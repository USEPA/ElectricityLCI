...
def generate_upstream_nuc(year):
    ...
    nuc_generation_data['Plant Id'] = nuc_generation_data['Plant Id'].astype(int)
    ...
    nuc_merged = pd.merge(
            left=nuc_lci, right =nuc_generation_data,
            left_on=['Plant Id'], right_on=['Plant Id'],
            how='left'
    )
    
    nuc_merged=nuc_merged.rename(
            columns={'Net Generation (Megawatthours)':'quantity'})
    
    #Convert the inventory per MWh emissions to an annual emission
    nuc_merged['FlowAmount']=nuc_merged['FlowAmount']*nuc_merged['quantity']
