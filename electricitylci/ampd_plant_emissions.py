...
"""
__all__ = [
    "eia_boiler_co2_ch4_n2o_emissions",
    "eia_boiler_nox",
    "eia_boiler_nox_emissions",
    "eia_boiler_so2_emissions",
    "eia_gen_fuel_co2_ch4_n2o_emissions",
    "eia_gen_fuel_net_gen",
    "eia_gen_fuel_nox_emissions",
    "eia_gen_fuel_so2_emissions",
    "eia_primary_fuel",
    "eia_wtd_sulfur_content",
    "emissions_check_boiler",
    "emissions_check_gen_fuel",
    "emissions_logic_CO2",
    "emissions_logic_NOx",
    "emissions_logic_SO2",
...
'''dict : Flow compartment map for air emissions'''
FUELCAT_MAP = {
    "AB": "BIOMASS",
    # "BFG",
    "BIT": "COAL",
...
'''dict : Fuel category shortcode to eLCI fuel name mapping.'''
...
def eia_boiler_nox_emissions(eia923_boiler_firing_type, ef_nox, eia_nox_rate):
    """Add docstring."""
...
def eia_gen_fuel_co2_ch4_n2o_emissions(eia923_gen_fuel,
    ...
    eia923_gen_fuel_sub :
        Subset of EIA 923 generation fuel data.
...
def eia_gen_fuel_nox_emissions(eia923_gen_fuel_sub, ef_nox):
    """Add docstring."""
    emissions = pd.DataFrame()
...
def eia_primary_fuel(row, model_specs):
    """Add docstring."""
...
def eia_wtd_sulfur_content(eia923_boiler):
    ...
    quantities and sulfur content reported in 'EIA-923 Monthly Boiler
    Fuel Consumption and Emissions Time Series File'. This approach implicitly
...
def emissions_check_gen_fuel(df, eia923_gen_fuel_sub_agg):
    """Unused."""
...
def emissions_logic_NOx(row):
    ...
    if (
        (
            row["ampd Heat Input (MMBtu)"]
            < row["ampd Heat Input (MMBtu)"] * 1.2
        )
        | (
            row["ampd Heat Input (MMBtu)"]
            > row["ampd Heat Input (MMBtu)"] * 0.8
        )
    ) & (
...
def generate_plant_emissions(year):
    ...
    # UserWarning associated with this file; likely an XML thing; no data loss
    ...
    eia_gen_fuel_net_gen_output = eia_gen_fuel_net_gen_output.assign(
        Primary_Fuel=eia_gen_fuel_net_gen_output.apply(
            lambda row: eia_primary_fuel(row, model_specs), axis=1
        )
    )
    ...
    eia_gen_fuel_co2_ch4_n2o_output = eia_gen_fuel_co2_ch4_n2o_emissions(
        eia923_gen_fuel, ef_co2_ch4_n2o, eia923_gen_fuel_sub
    )
    logger.info("Generating so2 emissions from gen fuel")
    eia_gen_fuel_so2_output = eia_gen_fuel_so2_emissions(
        eia923_gen_fuel_sub, ef_so2, wtd_sulfur_content_fuel
    )
    logger.info("Generating nox emissions from gen fuel")
    eia_gen_fuel_nox_output = eia_gen_fuel_nox_emissions(
        eia923_gen_fuel_sub, ef_nox
    )
    logger.info("Generating co2, ch4, n2o emissions from boiler")
    eia_boiler_co2_ch4_n2o_output = eia_boiler_co2_ch4_n2o_emissions(
        eia923_boiler, ef_co2_ch4_n2o, eia923_boiler_sub
    )
    # This seems to be the long one.
    logger.info("Generating so2 emissions from boiler fuel")
    eia_boiler_so2_output = eia_boiler_so2_emissions(
        eia923_boiler_firing_type, ef_so2, eia_so2_rem_eff
    )
    logger.info("Generating nox emissions from boiler fuel")
    eia_boiler_nox_output = eia_boiler_nox_emissions(
        eia923_boiler_firing_type, ef_nox, eia_nox_rate
    )
    ...
    netl_harmonized_melt['Flowable'] = netl_harmonized_melt['Flow'].map(
        flow_dict
    )
    ...
    #     ] = netl_harmonized_melt.loc[
    #         netl_harmonized_melt["FuelCategory"] == "PrimaryFuel"
    #     ]
